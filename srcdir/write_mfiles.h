...
/* macro for single liability class */

#define MENDEL7_AFFECTION       fprintf(fp, "%s,",Locus->Name);         \
    ((sex_linked==1)?                                                   \
     fprintf(fp, "X-LINKED,") : fprintf(fp, "AUTOSOME,"));              \
    fprintf(fp, "%d,2\n", Locus->AlleleCnt);                            \
    for (allele = 0; allele < Locus->AlleleCnt; allele++)  {            \
...
#define MENDEL7_ML_AFFECTION          fprintf(fp, "%s,",Locus->Name);   \
    ((sex_linked == 1)?                                                 \
     fprintf(fp, "X-LINKED,") : fprintf(fp, "AUTOSOME,"));              \
    fprintf(fp, "%d,2\n", Locus->AlleleCnt);                            \
    for (allele = 0; allele < Locus->AlleleCnt; allele++)  {            \
