...
// 'markers_on_chromosome()' called from 'loop::loci::run_loop()' lies, it will return all loci
...
// to just get the markers. 'loop::loci::run_loop()' "deals with this" by throwing away
...
/**
 ...
 loop.cpp:loop::loci::run_loop. However, a static method with access to a global 'linkage_ped_top *'
                                     ...
                                     const int pwid, const int fwid)
{
    ...
    if (base_pair_position_index >= 0) {
        ...
        struct save_base_pair_position_markers: public loop::chr, loop::loci {
            typedef char *str;
            str *file_names;
            save_base_pair_position_markers(linkage_ped_top *Top) : entry(Top), loop::chr(Top), loop::loci(Top) { }
            void make_file() {
    ...
    if (genetic_distance_index >= 0) {
        // If we have the data create the file...
        struct save_genetic_distance_markers: public loop::chr, loop::loci {
            typedef char *str;
            str *file_names;
            
            save_genetic_distance_markers(linkage_ped_top *Top) : entry(Top), loop::chr(Top), loop::loci(Top) { }
            void make_file() {
                if (_numchr > lastautosome) return; // because we have already issued a warning.
...
{
    // Pedigree...
    // 'loop::outer' loops over chromosome, and then trait...
    struct save_fams: public loop::outer, loop::ped_per {
        typedef char *str;
        str *file_names;
        int missing_pheno;

        save_fams(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per(Top) { }
        void make_file() {
    ...
    struct save_pers: public loop::outer, loop::ped_per {
        ...
        save_pers(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
    ...
    // Father...
    struct save_fathers: public loop::outer, loop::ped_per {
        typedef char *str;
        str *file_names;
        
        save_fathers(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
    ...
    // Mother...
    struct save_mothers: public loop::outer, loop::ped_per {
        typedef char *str;
        str *file_names;
        
        save_mothers(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
    ...
    struct save_sexs: public loop::outer, loop::ped_per {
        ...
        save_sexs(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
    ...
    struct save_phenotypes: public loop::outer, loop::ped_per {
        ...
        save_phenotypes(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
    ...
    struct save_markers: public loop::outer, loop::loci_ped_per {
        ...
        save_markers(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::loci_ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
                                                     ...
                                                     const int pwid, const int fwid)
{
    ...
    struct save_fams: public loop::outer, loop::ped_per_trait {
        ...
        save_fams(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per_trait(Top) { }
        void make_file() {
            if (_numchr > lastautosome) {
                warnf("Beagle analysis can only be run on autosomes.");
                return;
    ...
    struct save_pers: public loop::outer, loop::ped_per_trait {
        ...
        save_pers(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per_trait(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
    ...
    struct save_sexs: public loop::outer, loop::ped_per_trait {
        ...
        save_sexs(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::ped_per_trait(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
    ...
    struct save_phenotypes: public loop::outer, loop::trait_ped_per {
        ...
        save_phenotypes(linkage_ped_top *Top) : entry(Top), loop::outer(Top), loop::trait_ped_per(Top) { }
        void make_file() {
            if (_numchr > lastautosome) return; // because we have already issued a warning.
            run_loop(*_opath, file_names[0], "a");
        }
                            ...
                            char *file_names[])
{
    ...
    struct BEAGLE_sh_script: public loop::outer, all_sh {
        typedef char *str;
        str *file_names;
        all_sh *sh;
        analysis_type *analysis;
        
        BEAGLE_sh_script(linkage_ped_top *Top, analysis_type *analysis) : entry(Top), loop::outer(Top), all_sh(Top) {
