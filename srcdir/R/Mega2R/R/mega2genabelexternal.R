...
################################################################
################################################################
# ~/rvb/Work/R/pkg/GenABEL/R/load.gwaa.data.R 5609 Feb 19 13:47 load.gwaa.data.V1

"V1.gwaa.data.Check.Phe" <-
function(dta, id) {
	coln <- names(dta)
	idColumn <- match(id,coln)
	names(dta)[idColumn] <- "id"
...
"V1.gwaa.data.Epilog" <-
function(a, newdta, force = TRUE, makemap=FALSE, sort=TRUE) {
#check X chromosome markers
 	if (any(a@chromosome == "X") && any(a@male == 1) && !force) {
		xmrk <- (a@chromosome == "X")
		mlst <- (a@male == 1)
...
# check that ID and SEX are correct
	dta <- read.table(phenofile,header=TRUE,as.is=TRUE)
        dta <- V1.gwaa.data.Check.Phe(dta, id)
...
# read in genotypic data
	...
	rdta <- new("snp.mx",rdta);gc(verbose=FALSE)
        ...
        newdta <- V1.gwaa.data.Check.Persons(dta, ids)
	gc(verbose=FALSE)
	...
	rm(rdta,ids,mnams,chrom,pos,coding,strand);gc(verbose=FALSE)
        ...
        V1.gwaa.data.Epilog(a, newdta, force = force, makemap=makemap, sort=sort)
}


"V1.gwaa.data.mega2" <-
function(markers, force = TRUE, makemap=FALSE, sort=TRUE, id="id", envir=ENV) {
# check that ID and SEX are correct
