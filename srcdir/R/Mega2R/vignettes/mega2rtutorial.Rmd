...
All the files for this vignette will be created in the directory, `r where_mega2rtutorial_data()`.
...
**Note: The directory name, `r where_mega2rtutorial_data()`, is generated randomly each time this
vignette is run.  You will not get the exact value shown.  You must either run the vignette
Rmarkdown text as is or cut and paste from it.**

**Note:  The directory name is actually the value of the R expression, where_mega2rtutorial_data().**
...
command at the Unix prompt in the directory containing tutorial data:
`r where_mega2rtutorial_data()`
...
library(Mega2R)
db = paste(where_mega2rtutorial_data(), "seqsimr.db", sep="/")
...
and finds the markers that fall within the bounds.
It then builds a genotype matrix for the samples and each marker in the range.
For all ranges that contain the same set of markers, only one genotype matrix is generated 
and the call back function evaluated once.
...
```
applyFnToRanges(DOcallback, ENV$refRanges, ENV$refIndices, envir = ENV)
applyFnToRanges(DOcallback, ENV$refRanges[ENV$refRanges$chromosome == 1,], ENV$refIndices, envir = ENV)
```
...
> applyFnToGenes(DOcallback, genes_arg = c('DISP1', 'KIF26B', 'AK5', 'ST7L'), envir = ENV)

will find all the transcripts for each gene requested from the Bioconductor database.  It will then
call `applyFnToRanges`: find all the markers in each transcript range, compute the genotype
...
or collections of markers.  All these arguments define ranges that are passed to *applyFnToRanges*
for evaluation.
Note: If the *genes_arg* argument is set to the special "gene" string "*", then all 
transcripts in the Bioconductor database, will be processed.
...
You may specify a "filename" argument to `init_pedgene` to indicate where to store the 
data frame that is produced later.  (The default filename is NULL which indicates that the 
data frame should not be written.)

```{r init_pedgene,message=FALSE}
db = paste(where_mega2rtutorial_data(), "seqsimr.db", sep="/")
pedfile = paste(where_mega2rtutorial_data(), "pedgene.txt", sep="/")
ENV = init_pedgene(db, filename = pedfile)
```
...
nucleotides  1/1, 1/2 and 2/2 to the values 0, 1 and 2 (or 2, 1, 0) if 1/1 is the
...
# we will skip this line for the Rmd document production because it takes too long
applyFnToGenes(DOpedgene, envir = ENV)
```
...
You may specify a "filename" argument to `init_pedgeneSKAT` to indicate where to store the 
data frame that is produced later.  (The default filename is NULL which indicates that the 
data frame should not be written.)

```{r init_SKAT,message=FALSE}
db = paste(where_mega2rtutorial_data(), "seqsimr.db", sep="/")
skatfile = paste(where_mega2rtutorial_data(), "SKAT.txt", sep="/")
ENV = init_SKAT(db, filename = skatfile, verbose = F, allMarkers = F)
```
...
db = paste(where_mega2rtutorial_data(), "seqsimr.db", sep="/")
skatfile = paste(where_mega2rtutorial_data(), "SKAT.txt", sep="/")
ENV = init_SKAT(db, filename = skatfile, verbose = T, allMarkers = F)
```
...
The function, `Mega2SKAT`, is Mega2's interface to SKAT, both to `SKAT_Null_Model` and
...
function also defines a callback function that converts the genotype
information from the forms 1/1, 1/2, and 2/2 to 0, 1, and 2 with the
Major allele (flipped if neccessary to be) 0 and then calls the
...
If you are adventurous you can look at all the transcripts; try:
...
```{r mega2vcflib}
vcfdir = paste(where_mega2rtutorial_data(), "vcfr", sep="/")
...
```{r mega2vcf}
vcffile = paste(where_mega2rtutorial_data(), "vcfr", "vcf.01", sep="/")
...
```{r lsseq}
path = tempdir()
list.files(path=path, pattern = "Mega2GenABEL.*")
Mega2GenABELClean()
```
...
At the Unix command prompt in the directory, `r where_mega2rtutorial_data()`, type:
...
```{r dmpPed}
srdtafile = paste(where_mega2rtutorial_data(), "srdta", sep="/")
dmpPed(gwaa_ = srdta, pfx = srdtafile)
```

To produce the Mega2 database `srdta.db`, we switch to the Unix command line, cd to
the directory, `r where_mega2rtutorial_data()`, and run:
...
sdb = paste(where_mega2rtutorial_data(), "srdta.db", sep="/")
