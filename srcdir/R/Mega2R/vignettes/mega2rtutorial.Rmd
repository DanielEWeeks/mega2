...
# 10 Converting Mega2R data to GDS (CoreArray) format
...
is a C++ package available from SourceForge^[http://sourceforge.net] as http://corearray.sourceforge.net.
...
package 
`gdsfmt` (http://bioconductor.org/packages/gdsfmt) provides a machine independent 
...
SeqArray (http://www.bioconductor.org/packages/SeqArray)
uses the 
...
SNPRelate (http://www.bioconductor.org/packages/SNPRelate).)
...
## 10.1 CoreArray features vs Mega2R database features 
...
## 10.2 Creating a GDS file using Mega2R
...
### 10.2.1 Creating the GDS file in SeqArray gdsfmt format
Type:
...
closefn.gds(gdsn)
```
### 10.2.2 Creating the SNPRelate file in SNPArray gdsfmt format
Type:
