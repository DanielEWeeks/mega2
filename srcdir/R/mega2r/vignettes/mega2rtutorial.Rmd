...
addition, we have developed C++ functions for R to decompress needed
subsets of the genotype data, on the fly, in a memory efficient manner.  
We have also created several more R functions that
illustrate how to use the data frames as well as perform useful
functions: these permit one to run the 'pedgene' R package (https://CRAN.R-project.org/package=pedgene) to carry
out gene-based association tests on family data using selected
marker subsets, to output subsets of the 'mega2r' data as a VCF file (https://github.com/samtools/hts-specs) and
related files (for phenotype and family data), and to convert the
data frames into 'GenABEL' R gwaa.data-class objects (https://CRAN.R-project.org/package=GenABEL).

This tutorial shows how to read the SQLite
database into R and how to access tables in it using this package, `mega2r`.
...
locations and the other maps gene names to entrez gene IDs. (Note: As described in Section 5.3.1, you
...
Mega2R provides functions that permit one to run the `pedgene` R package to carry
...
genotype homozygous for the allele with the minor allele frequency.  Then it runs pedgene. 
...
The `Mega2VCF` function permits one to output subsets of the 'mega2r' data as a VCF file, accompanied by
...
## 7.1 Writing a VCF File using Mega2R
...
data frames into 'GenABEL' format (e.g. as gwaa.data-class objects).
...
## 8.1 Creating a GenABEL gwaa.data-class object using Mega2R
...
To verify that the R functions described above have worked correctly,
...
normalizes the two heterozygous cases to one value.  These results are
