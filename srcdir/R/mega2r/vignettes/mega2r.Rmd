...
package.  Most of the vignettes will access the data directly in **inst/doc**,
but building or running the
mega2rtutorial "vignette" and the running of the "mega2" executable, require
...
*mega2r* restrieves a Mega2 SQLite database into a collection of R data frames.
It is necessary and might be suffficent for your task.
*mega2vcf*, *mega2genabel* and *mega2pedgene* perform specific functions (see their package description);
these load the *mega2r* package when they are loaded.  Their code illustrates how the Mega2 data frames
can be used.
...
variable, **envir**.  Thus if **envir** is not defined,
...
called with four arguments: the genotype matrix, the markers, the selected transcript/range
...
Alternatively, you may try searching by gene transcript.  Here, the default
...
will find the transcripts for each gene in the Bioconductor database.  It will then behave
like *applyFnToRanges*, find all the markers in each transcript range and then invoke the call
back function with the required four arguments.

*applyFnToGenes* allows several other arguments that specify complete chromosomes, or
...
The heart of the above two functions is the calculation of the matrix of samples by markers
containing the nucleotides (strings).  The
genotype information is most likely stored in a compressed 2 bit representation.  An Rcpp function does the
...
for each genotype.  The integer is
