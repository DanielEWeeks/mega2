...
package.  Most of the Mega2R vignettes will access the data directly in **inst/exdata**.
But building or running the
mega2rtutorial "vignette" and the running of the "mega2" executable chunks, require
...
*mega2rtutorial* is needed to retrieve data and to create a complete vignette/tutorial.
*mega2r* reads a Mega2 SQLite database into a collection of R data frames.
This package is needed for all other packages and might, alone, be suffficent for your task.
*mega2vcf*, *mega2genabel* and *mega2pedgene* perform specific functions (see their package description);
they will load the *mega2r* package when they are loaded.  Their code illustrates how data can
be extracted from the the Mega2 data frames.

Mega2R also requires that Bioconductor Annotations be loaded.
...
We have chosen to use the SeqSIMLA2 program to simulate a data set for
these exercises.  We needed to sub-sample the data down to 1,380 people and 1,000
...
on your data. To make the tutorial simple, we will use a pre-constructed
...
When you are ready, you will have to run Mega2 to convert your data to a database.
...
environment" that contains the database tables as data frames is returned. (If you
...
If verbose is TRUE, Mega2R functions may print out more
...
environment) has a named argument (**envir**) defined as
...
variable, **envir**.  Thus if **envir** is not provided,
**ENV** (from the global environment) will be used.
...
nucleotide patters to the values 0, 1, and 2 with 0
assigned to the  minor allele and it then runs CRAN pedgene for
...
Alternatively, you may try searching by genes.  Here, the default
...
returns the matrix for the specified markers.  It can take one additional argument that supplies a string to 
separate the two alleles of each marker.
...
is similar to the *getgenotypes* function except that the matrix it returns contains an integer
encoding each genotype.  The integer's
