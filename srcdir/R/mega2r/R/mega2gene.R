...
#'  different father/mother.  This the number of persons per family will be different when there
...
#'
#' @note
#' The columns of this data frame are from the \emph{pedigree_table}, \emph{person_table}, and \emph{trait_table}.
#'
#' @note
#' Also, the father and mother columns from \emph{person_table} are translated from a row index in the
...
#'  table in the environment.  Additionally,
#'  changing \emph{fam} data frame will filter the genotypes data frame to only contain persons
#'  matching those in the \emph{fam} data frame.  It does the same filtering for the phenotype data
#'  records.
#'
...
#'
#' @param ... additional arguments to pass to \emph{dbmega2_import}
#'
...
#' apply a function to genotypes (markers) in each gene transcript
#'
#' @description
#' The function \code{op} will be called for every set of markers that is specified by the 
#'  arguments of \code{applyFnToGenes}.
#'
#' @usage
...
#'  the genotypes, markers, range, and environment.
#' @note
#'  If it is desired that subsequent calls to \emph{op} share information, this data can be placed in
...
#' set default ranges: chromosome and start and end base pair for each element in the default range data
#'
#' @description
#'  This function sets the default list of ranges used by \emph{applyFnToRanges}.  \emph{applyFnToRanges}
...
#'   one to specify which Bioconductor gene name to transcripts,
...
#' apply a function to all the genotypes in each of several specified ranges
#'
#' The function \code{op} will be called for every set of markers that is falls in each range.
#'  This function is called by \code{applyFnToGenes}.
#'
#' @usage
...
#'  pair column and end base pair column of the range data frame.
#'
#' @param fuzz_arg is an integer vector of length one or two.  The first argument is used to reduce
...
#'
#' @description
#'  A matrix of the genotypes for all the markers is generated.  Then, the call back function, \emph{op},
...
#'  }
#' }
#' \item{range}{NULL: to indicate no range in use.}
#' \item{envir}{An R environment holding Mega2 data frames and state data.}
#' }
#'
#' @param markers_arg a data frame with the following 5 variables:
