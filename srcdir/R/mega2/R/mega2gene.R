...
setfam = function (fam) {
    ...
    ENV$phenotype_table = ENV$phenotype_table[(ENV$phenotype_table$person_link %in% fam[ ,1]), ]
    row.names(ENV$phenotype_table) = NULL
...
#'
#' @usage
#' applyFnToGenes(op           = function (geno, markers, range) {},
...
applyFnToGenes = function (op = function (geno, markers, range) {},
    ...
    env1=loadNamespace(ENV$txdb)
    txdb = get(ENV$txdb, env1)
    if (type_arg == "TX")
    ...
    env2=loadNamespace(ENV$entrezGene)
    genedb = get(ENV$entrezGene, env2)
...
#           ENTREZID  ALIAS SYMBOL  TXID     TXNAME TXCHROM TXSTRAND  TXSTART    TXEND
    ...
    applyFnToRanges(op, ranges, c(6, 8, 9))
...
#'
#' @usage
#' applyFnToRanges(op          = function (geno, markers, range) {},
#'                 ranges_arg  = NULL,
#'                 indices_arg = NULL,
#'                 fuzz_arg    = 0 )
#' 
...
applyFnToRanges = function (op          = function (geno, markers, range) {},
    ...
    if (rows) {
        ...
        for (i in 1:rows) {
            ...
            markersub = ENV$markers[ ENV$markers$chromosome == chrm[i] & ENV$markers$position <= (end[i]) & ENV$markers$position >= (start[i]), ]

            if (nrow(markersub)) {
                geno = getgenotypes(markersub)
                op(geno, markersub, ranges[i,])
            } else {
                if (ENV$verbose)
...
#'
#' @usage
#' applyFnToMarkers(op      = function (geno, markers, range) {},
#'                 markers_arg)
#'
#' @param op Is a function of three arguments.  It will be called repeatedly by
#' \code{applyFnToGenes}.  The arguments are:
...
applyFnToMarkers = function (op = function (geno, markers, range) {},
    ...
    geno = getgenotypes(markers_arg)
    op(geno, markers_arg, NULL)
