...
following this example illustrated with *seqsimr.db*.
...
file are processed by the function, *load.gwaa.data*, to yield a gwaa.data-class
object.
...
be used to name the .tped/.fam/.phe files that are created.
The GenABEL functions *convert.snp.tped* and *load.gwaa.data* are explained in
...
This test is a bit complicated.  We intend to verify that the object generated 
...
These results are somewhat disappointing.  The first difference in **gtdata\@strand** occurs
because Mega2 does not keep track of strand orientation and always returns 0.  The second
difference is more subtle.  **gtdata\@coding** and **gtdata\@gtps** show the allele pair and genotype
in an encoded fashion.  And they are different.  **gtdata** with no trailer, decodes the genotype
to characters then normalizes the two heterozygous case to one value.  These fields are the same
for the two different gwaa.class-object; the data are really the same.  The issue with the
encoded values is that the allele order of 1 vs 2 occassionally gets flipped.  In principle, this
should not happen because the major allele should appear first.  It does for the newly generated
object, but for the *srdata* object this order is not always observed.
