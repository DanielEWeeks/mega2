...
the data that Mega2 has processed in to R.  This is what Mega2R and
...
the pedgene R package (https://CRAN.R-project.org/package=pedgene) and another package, `mega2genabel` to interact with
the GenABEL R package.
A final package, `mega2vcf`, writes subsets of the database into
VCF 
format files (https://github.com/samtools/hts-specs).
...
To run any of these
exercises, you should install the R packages `mega2r`, `mega2vcf`,
...
In the `mega2pedgene` package, we will carry out gene-based association tests, where 'genes' are defined according to a database containing the boundaries of the gene transcripts.
...
two lines install two annotation databases. One annotation databases provides the gene transcript
...
The output seen on the screen when we ran Mega2 to create the SQLite3 database is as follows:
...
If you wish to use any of the Mega2R suite of R packages, you will initially have to run Mega2 to convert your data into an SQLite database.

## 5.2 Reading and Examining a Mega2 Database in R

The `mega2r` R package facilitates reading genetic data into R from a Mega2-created SQLite database.

### 5.2.1 Reading a Mega2 database into R

After you have created the SQLite database, start up the R program. Load the `mega2r` R package, then
...
The `pedgene` R package implements methods for carrying out gene-based association tests on family data, and is available on CRAN as (https://CRAN.R-project.org/package=pedgene).
...
And then, to use the desired transcription data base, use this command from the `mega2r` R package as part of your R session:
```
...
The GenABEL package is available on CRAN as (https://CRAN.R-project.org/package=GenABEL).
The reference is 
...
the same for the two different gwaa.class-objects; the data are really
