...
We require two Bioconductor Annotations that must be loaded
...
available.  This is true for all the invocations of mega2 shown in these exercises.**
...
faster, we noticed that the gene appeared at transcript 54; so we will
...
In addition, you need to load the database with biocLite.
...
it generates. Here, we have chosen to make that a path that includes a
directory.  The Mega2VCF function assumes the Mega2 data frames
...
Note: The line above only creates files for chromosome 1.  You can filter ENV$markers as you find necessary.
If the second argument is not present, all the markers are dumped.
...
are read by GenABEL (https://cran.r-project.org/package=GenABEL/)
