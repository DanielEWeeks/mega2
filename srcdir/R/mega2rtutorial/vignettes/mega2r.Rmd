...
created Mega2.  The Mega2 Quick Start tutorial will help you get started using Mega2.
...
locus pentrance information can be specified. The input data are read
and validated once, then stored in a SQLite3 database file.

Mega2 then takes the database file and, via a menu-driven interface,
transforms it into various other file formats (listed in Section
[subsec:Output-formats]), thus greatly facilitating a variety of
...
1. The ability to create publication-quality PDF plots of the results using our nplplot R library.
...
The features listed above and the tutorial,
https://watson.hgen.pitt.edu/docs/mega2_html/mega2.html, are forthe
Mega2 executable, written in C++.  The site
https://watson.hgen.pitt.edu/register/ provides binaries for a number
...
https://bitbucket.org/dweeks/mega2.)

Since Mega2 now produces a SQLite database, it is now easy to load
the GWAS data that Mega2 has processed in to R.  This is what Mega2R and
this tutorial is all about.  This tutorial shows how to read the SQLite
database into R and how to access tables in it using the package, `mega2r`.
The tutorial shows how to select subsets
of the data that corresponds to transcripts or other base pair ranges
and perform analysis on them.  It is important to point out that
...
when needed.  Mega2R also provides a package, `mega2pedgene` to interact with
CRAN Pedgene (https://CRAN.R-project.org/package=pedgene) and another package, `mega2genabel` to interact with
CRAN GenABEL.
A final package, `mega2vcf`, writes subsets of the database into
VCF (https://github.com/samtools/hts-specs)
format files.

## 1.3 On to the tutorial



# 2 Example GWAS Data

We have chosen to use the SeqSIMLA2 program to generate a data set for us.
...
**We will assume that you have started an R session at which to type the commands
in the tutorial.**

To run any of these
exercises, you should install the CRAN packages `mega2r`, `mega2vcf`,
`mega2genabel`,  mega2pedgene` and `mega2rtutorial`

```{r, install,eval=FALSE}
install.packages("mega2r")
install.packages("mega2pedgene")
install.packages("mega2vcf")
install.packages("mega2genabel")
install.packages("mega2rtutorial")
```

Alternatively, if you are running from source code, you may issue these commands
at the UNIX prompt:
...
In the `mega2pedgene` package, we will process data by gene transcripts.
...
locations and the other to map gene names to entrez gene IDs. (Note: You
...
database that is in the mega2rtutorial package.**
...
If you wish to use any of the Mega2R suite of R packages, you will eventually have to run Mega2 to convert your data into an SQLite database.

## 5.2 Reading and Examining a Mega2 Database in R

### 5.2.1 Reading a Mega2 database into R

After you have the database, start up the R program. Load the *mega2r* library, then
use the function *read.Mega2DB* to read a Mega2 database into R.
...
### 5.2.5 Use standard R commands to examine the created data frames
...
There are two ways to compute a function on all the genotypes (or markers) in a
transcript.
These examples are explained more thoroughly in the `mega2pedgene` package,
Section 6.)
...
It then builds a genotype matrix for the samples and each marker in the range.

The first argument of the *applyFnToRanges* function is a call back function.  The function is
called with four arguments: the genotype matrix, the markers in range, the selected transcript/range
entry and the environment.  The call back is invoked repeatedly for each transcript range that contains any markers.
If it is necessary to store information between successive invocations the environment (`envir`) 
can be used.

*applyFnToRanges* can also be provided explicit ranges.
```
...
### 5.3.2 applyFnToGenes

Alternatively, you may search by genes.  Here, the default
transcript database is "TxDb.Hsapiens.UCSC.hg19.knownGene" from
Bioconductor.  Of course you can change it, by typing:
...
The `setAnnotations` command must be issued whenever R is started and the `mega2r`
library has been loaded.  By way of a reminder, section 4.3  "Installation.Bioconductor",
explains how to choose and install "TxDb.Hsapiens.UCSC.hg19.knownGene" or a different
Annotation database.

Note: We could require that you add to your R session
```
library(<name of the transcription database>
```
but we have chosen not to.  Instead, you just indicate the database name and the 
logic of `applyFnToGenes` does a `loadNamespace` of the string and then a `get` of
the string from the namespace.
...
will find all the transcripts for each gene requested from the Bioconductor database.  It will then behave
like `applyFnToRanges`: find all the markers in each transcript range, compute the genotype
matrix and then invoke the call
back function with the required four arguments.

The *applyFnToGenes* function has several other optional arguments that can
specify complete chromosomes,
(multiple) ranges of base pairs on chromosomes,
or collections of markers.

## 5.4 Iterating through the Genotypes

### 5.4.1 Cooked Genotypes

The heart of the above two functions is the calculation of the genotype 
matrix of samples by markers
containing the nucleotides (strings).  The
genotype information is mostly stored in a compressed 2 bit representation.
...
# 6 Using the `mega2pedgene` package

The pedgene packages is available on CRAN as (https://CRAN.R-project.org/package=pedgene).
...
## 6.1 Gene transcripts reminder

If you plan to select transcripts by gene name, you must loaded them from
Bioconductor.  In Section 4.2, "Installation Bioconductor", we indicated that you 
needed to type once in R to install the package:
...
We also indicated that you could select any database to install provided you type
the command once to install it:
```
biocLite("transcript database name")
```
And then type the command as part of your R session:
```
setAnnotations("transcript database name", "gene map database name")
```

## 6.2 Loading a Mega2 database
...
### 6.3 Running pedgene on transcripts
...
### 6.4 Running pedgene on selected genes
...
## 7.1 Writing a VCF File using the mega2vcf R package

First, load the mega2vcf library and then create the directory "vcfr",
...
# 8 Using the `mega2genabel` package

The GenABEL packages is available on CRAN as (https://CRAN.R-project.org/package=GenABEL).
The reference is 
...
## 8.1 Creating a GenABEL gwaa.data-class object using the mega2genabel R package
...
object.
The argument to `Mega2GenABEL` is a file name prefix that will
be used to name the .tped/.fam/.phe files that are created.  The
...
Note: We have provided a copy of the srdta.db database in the `mega2rtutorial`.  So
you can proceed with the steps that follow, even if you did not create your own database.

Now, we go back into R and type:
...
The second difference is more subtle.  It arises because we convert
new data using the `convert.snp.tped` function while the `srdta`
object is generated by GenABEL using the `convert.snp.text`.  The
**gtdata** comparison line decodes the genotype to characters then
normalizes the two heterozygous case to one value.  These results are
the same for the two different gwaa.class-object; the data are really
the same.  But though both functions encode the genotype to a number,
the `convert.snp.tped` function rearranges the genotype so that
the major allele appears first, the `convert.snp.text` does no such
rearrangement.  So because **gtdata\@coding** and **gtdata\@gtps** compare the
encoded allele pairs and corresponding genotypes after encoding, they
